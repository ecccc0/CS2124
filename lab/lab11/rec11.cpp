...
Node* reverse(const Node* hp)
{
    ...
    for (const Node* p = hp; p != nullptr; p = p->next) {
        newHead = new Node{p->data, newHead};
    }
    return newHead;
...
const Node* isSublist(const Node* pattern, const Node* target)
{
    ...
    for (const Node* p = target; p != nullptr; p = p->next) {
        const Node* pt = p;
        const Node* pp = pattern;
        while (pt != nullptr && pp != nullptr && pt->data == pp->data) {
            pt = pt->next;
            pp = pp->next;
        }
        if (pp == nullptr) {
...
{
    for (const Node* pB = listB; pB != nullptr; pB = pB->next) {
        for (const Node* pA = listA; pA != nullptr; pA = pA->next) {
            if (pA == pB) {
                return pA;
            }
        }
...
const Node* sharedListUsingSet(const Node* listA, const Node* listB)
{
    ...
    for (const Node* pB = listB; pB != nullptr; pB = pB->next) {
        if (nodeSet.find(pB) != nodeSet.end()) {
            return pB;
        }
    }
    return nullptr;
