...
class Person {  
   public:       
       ...
       void movesInWith(Person& newRoomate) {
         if (this != &newRoomate && this->roomie == nullptr && newRoomate.roomie == nullptr) {
...
int main() {
   ...
   // p = 0x000000016fdff20c;

   // incompatible integer to pointer conversion assigning to 'int *' from 'long'
   ...
   *pI = 234;  
   // pI = &sal;  
   // &sal is a reference to a const int, pI is non const
   *pI = 7623;    

   const int* pcI;
   pcI = &joe;  
   // *pcI = 234;  
   // * operator could not be used on a pointer to const value
   pcI = &sal;  
   // *pcI = 7623;    
   // * operator could not be used on a pointer to const value 

   // int* const cpI;
   // const pointer needs to be initialized
   int* const cpI(&joe);
   // int* const cpI(&sal);
   // sal is a const value so can't be associated with a pointer to non-const
   // cpI = &joe; 
   // can not change the value of a const ptr 
   *cpI = 234;  
   // cpI = &sal;  
   // can not change the value of a const ptr
   *cpI = 7623;    
   ...
   // const ptr needs to be initialized
   const int* const cpcI(&joe);
   // const int* const cpcI(&sal);
   // redefinition
   // cpcI = &joe;  
   // cannot assign values to a const ptr
   // *cpcI = 234;  
   // cannot use * operator on a ptr to const value
   //  cpcI = &sal;  
   // cannot assign values to a const ptr
   // *cpcI = 7623; 
   // cannot use * operator on a ptr to const value
